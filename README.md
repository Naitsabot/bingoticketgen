...
Generation of bingo tickets that are 3x9 with 15 numbers, min 1 and max 2 numbers per column.

`pdfgen` contains `PDF_ticket_generator()` that can produce `n` number of bingo tickets. There are 4 tickets per page.

`ticketgen` contains `ticket_generator()` which generates a 2dimentional list representing a bingo ticket.
